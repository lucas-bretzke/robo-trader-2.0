...
logger = logging.getLogger("robo-trader.trader")

def connect_to_iqoption(email=None, password=None):
    """Connect to IQ Option platform"""
    logger.info("===== IQ OPTION TRADER =====")
    ...
    if check:
        ...
        try:
            if isinstance(reason, str) and "invalid_credentials" in reason:
                ...
                logger.error(f'Connection problem: {reason}')
...
def main():
    ...
    try:
        ...
        if not api:
            print("Failed to connect to IQ Option. Exiting...")
            return
